...
RSpec.describe Rblox::Parser do
  before(:all) do
  ...
  it "has a version number" do
    expect(Rblox::Parser::VERSION).not_to be nil
  end
  ...
  it "parses a simple mathematical expression" do
    ...
    printed_expressions = statements.map do |statement|
      Rblox::Parser::AstPrinter.new.print(statement.expression)
    end
  ...
  it "parses multiple statements" do
    ...
    statements = Rblox::Parser::RecursiveDescentParser.new(tokens, @error_handler).parse!
    printed_expressions = statements.map do |statement|
      Rblox::Parser::AstPrinter.new.print(statement)
    end
